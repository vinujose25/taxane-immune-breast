...
xout[["ddfs"]] <- summarize_tn(x = finher_tn$prog$DDFS, prog = T)
xout[["os"]] <- summarize_tn(x = finher_tn$prog$OS, prog = T)
xout[["rfs"]] <- summarize_tn(x = finher_tn$prog$RFS, prog = T)

write_xlsx(xout, path = str_c(out_tables,"finher_tn_prognosis_summary.xlsx"))


xout <- list()

xout[["ddfs"]] <-  summarize_tn(x = finher_tn$inter.chemo$DDFS, prog = F)
xout[["os"]] <- summarize_tn(x = finher_tn$inter.chemo$OS, prog = F)
xout[["rfs"]] <- summarize_tn(x = finher_tn$inter.chemo$RFS, prog = F)

write_xlsx(xout, path = str_c(out_tables,"finher_tn_chemo_interaction_summary.xlsx"))
