...
# 1. Sig's prognostic value in HR +/- subgroups in HER2+BC (with arm heterogenity; interaction with *four* stratas)
...
finher_her2_subgroup <- list()

# HER2 prognosis
# >>>>>>>>>>>>>>

finher_her2_subgroup[["prog"]] <- purrr::map(
...
names(finher_her2_subgroup[["prog"]]) <- c("RFS", "DDFS", "OS")
...
summarize_her2 <- function(x, prog = F){
  # x <- finher_her2_subgroup$prog$DDFS
  # prog = T
  ...
  bind_cols(x) %>%
    dplyr::rename(Variable = "neg_Variable") %>%
  dplyr::select(-c(pos_Variable))

}

xout <- list()

xout[["ddfs"]] <- summarize_her2(x = finher_her2_subgroup$prog$DDFS, prog = T)
xout[["os"]] <- summarize_her2(x = finher_her2_subgroup$prog$OS, prog = T)
xout[["rfs"]] <- summarize_her2(x = finher_her2_subgroup$prog$RFS, prog = T)

write_xlsx(xout, path = str_c(out_tables,"finher_her2_subgroup_prognosis_summary.xlsx"))

# HR -ve
# In HR-HER2+ PH is valid for all models, but no signatures showed prognostic effect

# HR +ve
# In HR+HER2+ PH is valid for all models, except Denovo_TILsig and General_Immune.
# And no signatures showed prognostic effect.
...
save(finher_her2_subgroup, file = str_c(out_data,"finher_her2_subgroup.RData"))
