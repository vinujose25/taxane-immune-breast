# s9.2_geo_exploring_interaction_neo.R
        ...
        xhet <- purrr::map_dfr(
          ...
          function(l, xclin, sig){
            ...
            if (l == "") {
              ...
              # only one dataset has TRA containing regime. Hence dataset heterogeneity
              # assesment can't be done for this regimen. This situation will be
              # handled in estimate_prog_heterogeneity()
...
# geo_inter$pooled.sig$TN[["T.AAA_or_A0A"]] <- NA # supplementary analysis
# geo_inter$pooled.sig$TN[["Global"]] <- NA # supplementary analysis
      ...
      function(sig, xclin){
        ...
        xhet <- purrr::map_dfr(
          ...
          function(l, xclin, sig){
            ...
            if (l == "") {
              # estimate heterogeneity for the pooled dataset
              estimate_prog_heterogenity(
                xclin = xclin,
                sig = sig
              ) %>%
              ...
              # estimate heterogeneity per arm
              estimate_prog_heterogenity(
                xclin = xclin %>%
                  dplyr::filter(Arm_consolidated == l),
                sig = sig
...
geo_inter_neo <- geo_inter
save(geo_inter_neo, file = str_c(out_data,"geo_inter_neo.RData")) # old name: inter_sum
...
rm(clin_neoadj_neo, geo_inter_neo, geo_inter)
